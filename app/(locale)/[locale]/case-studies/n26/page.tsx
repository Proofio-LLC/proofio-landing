...
export default async function N26CaseStudyPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const messages = await getMessages(locale as any);
  const localePrefix = locale && locale !== "en" ? `/${locale}` : "/en";
  const content = getCaseStudyDetail((locale as any) || "en", "n26");
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <section className="mt-6 rounded-[2rem] border border-base-300 bg-base-100 p-8 md:p-12 shadow-lg">
            <div className="mb-6 flex items-center justify-between gap-3">
              ...
              <Image src="/n26.svg" alt="N26" width={84} height={28} className="h-7 w-auto" />
            </div>
